...
const statePublicationObserved =
  statePublicationFailed ||
  (Boolean(healthPath) &&
    (["success", "failure"].includes(process.env.STATE_STATUS_OUTCOME) ||
      ["success", "failure"].includes(process.env.STATE_PUBLICATION_OUTCOME)));
const observedFailureKinds = [
  ...(["success", "failure"].includes(process.env.ACTION_LEDGER_OUTCOME)
    ? ["action_ledger_failure"]
    : []),
  ...(statePublicationObserved ? ["state_publication_failure"] : []),
  ...(healthPath ? ["safe_close_blocked"] : []),
...
const failures = [
  ...
  ...(process.env.ACTION_LEDGER_OUTCOME === "failure"
