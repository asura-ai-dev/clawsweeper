...
test("target dispatcher documents opt-in cross-route identity", () => {
  ...
  assert.match(dispatcher, /## Cross-route exact-review identity \(maintainer decision required\)/);
  assert.match(dispatcher, /ingress_route:"target_dispatcher"/);
  assert.match(dispatcher, /ingress_fingerprint/);
  assert.match(dispatcher, /legacy-only delivery remains a safe fallback/);
  assert.match(dispatcher, /not a head-SHA/);
});
