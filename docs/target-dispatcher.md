...
## Cross-route exact-review identity (maintainer decision required)
...
change is therefore admitted rather than letting a stale direct decision hide
the fallback's current branch. A route with no valid fingerprint is deliberately
admitted, so a legacy-only delivery remains a safe fallback.

Adopting this protocol on a target repository is a maintainer decision: first
agree the event fields and retention policy, then roll out the dispatcher and
verify direct-only, legacy-only, cross-route duplicate, later body/revision, and
maintainer-command cases. The hash is an opaque queue receipt, not a head-SHA
dedupe key; body and metadata updates therefore produce a new event identity.
